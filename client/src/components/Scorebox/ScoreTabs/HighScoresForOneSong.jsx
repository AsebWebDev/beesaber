...
function HighScoresForOneSong(props) {
  ...
  if (highscores.playedByHive) {
    scoreColorClass =
      highscores.playedBy.filter((bee) => bee.myScore < bee.beeScore).length > 0
        ? "played-by-hive-lost"
        : "played-by-hive-won";
  }
  ...
  return (
    <tr className={scoreColorClass}>
      <td>
        <MDBBadge id="badge-light" color="light">
          {highscores.rank}
        </MDBBadge>
      </td>
      <td>
        <div className="song-data">
          ...
          <MDBBadge onClick={() => logid(highscores.songHash)} color="dark">
            {highscores.songAuthorName} - {highscores.songName}
            <span className="greyed-out"> by {highscores.levelAuthorName}</span>
          </MDBBadge>
        </div>
      </td>
      <td>
