...
function UserInfo(props) {
  ...
  const isAlreadyAdded = props.userdata.bees.some(
    (item) => item.playerId === playerId
  );

  const url =
    userInfoData.avatar === "/images/steam.png" ||
    userInfoData.avatar === "/images/oculus.png"
      ? avatarPlaceholder
      ...
      <div id="userinfo" className="col-md-3 card-container">
            ...
            <MDBTable striped>
              <MDBTableBody>
                <tr>
                  ...
                  {totalPlayCount && (
                    <td>
                      <MDBBadge color="light">{totalPlayCount}</MDBBadge>
                    </td>
                  )}
                  <td>PP</td>
                  <td>
                    <MDBBadge color="light"> {pp}</MDBBadge>
                  </td>
                </tr>
                <tr>
                  <td>Rank</td>
                  <td>
                    <MDBBadge color="light"> #{rank}</MDBBadge>
                  </td>
                  <td></td>
                  <td></td>
                </tr>
        ...
        <div>
          {handleChose && (
            <p id="plusicon">
              <MDBIcon
                onClick={() => handleChose(userInfoData)}
                icon="plus-circle"
                className={isAlreadyAdded ? "plusicon-grey" : ""}
              />
            </p>
          )}
        </div>
