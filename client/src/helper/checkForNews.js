...
export async function checkForNews(userdata) {
  ...
  for (let i = 0; i < bees.length; i++) {
    ...
    await api
      .getScoreSaberUserInfo(playerId, "id") // FETCH USERDATA for current Bee
      .then((userInfo) =>
        compareScores(userInfo, oldTotalPlayCount, playerId, playerName)
      );
  }
  ...
  async function compareScores(
    ...
    if (newTotalPlayCount !== oldTotalPlayCount) {
      // if different === new highscores exist
      const numPlayedMore = newTotalPlayCount - oldTotalPlayCount; // calc the difference
      if (userInfo)
        console.log(userInfo.playerName, " played ", numPlayedMore, " more!");
      await api
        .getScores(playerId) // FETCH SCORES for current Bee
        .then((scoreData) => {
          if (scoreData && scoreData.scoresRecent) {
            const { scoresRecent } = scoreData;
            news.push(
              new News({
                // Create a News with changes data of current bee
          ...
          bees = bees.map((bee) =>
            bee.playerId === playerId ? (bee = { ...userInfo, scoreData }) : bee
          );
        });
    }
  }
